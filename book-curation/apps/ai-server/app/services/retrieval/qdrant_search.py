...
class BookQdrantSearcher:
    """Qdrant 도서 검색 adapter입니다."""
    ...
    def __init__(
        ...
        *,
        ...
        self.fallback_collection_name = fallback_collection_name or settings.QDRANT_COLLECTION
    ...
    def search(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if not query:
            print(f"[VECTOR SEARCH] skipped empty query")
            return []
        ...
        if search_mode in {"author", "title", "isbn"}:
            ...
            if not keywords:
                print(f"[PRECISE SEARCH SKIPPED] mode={search_mode}, query={query!r}, reason=no keyword")
                return []
            ...
            if keyword_results:
                print(f"[PRECISE SEARCH HIT] mode={search_mode}, keywords={keywords}, count={len(keyword_results)}")
                return keyword_results

            print(f"[PRECISE SEARCH MISS] mode={search_mode}, keywords={keywords} → no vector fallback")
            return []

        print(f"[VECTOR SEARCH] mode={search_mode}, query={query!r}, limit={limit}")
    ...
    def search_by_intent(self, query_intent: QueryIntent, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if search_mode in {"author", "title", "isbn"}:
            ...
            if not keywords:
                print(f"[PRECISE SEARCH SKIPPED] mode={search_mode}, reason=no structured keyword")
                return []
            ...
            if keyword_results:
                print(f"[PRECISE SEARCH HIT] mode={search_mode}, count={len(keyword_results)}")
                return keyword_results

            print(f"[PRECISE SEARCH MISS] mode={search_mode} → no vector fallback")
            return []

        if not query:
            print(f"[VECTOR SEARCH] skipped empty structured query")
            return []
        print(f"[VECTOR SEARCH] mode={search_mode}, query={query!r}, limit={limit}")
        return self._search_by_vector(query=query, limit=limit)
    ...
    def search_dense_bm25_rrf(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        query = str(query or "").strip()
        if not query:
            print(f"[QDRANT HYBRID SEARCH] skipped empty query")
            return []
        return self._search_by_vector(query=query, limit=limit)
    ...
    def search_vector_only(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        query = str(query or "").strip()
        if not query:
            print(f"[VECTOR SEARCH][vector_only] skipped empty query")
            return []
        if self.hybrid_enabled:
            ...
            return self._search_dense_vector_on_collection(
                ...
                match_type="vector",
                cache_namespace="dense_only",
            )
        print(f"[VECTOR SEARCH][vector_only] collection={self.collection_name}, query={query!r}, limit={limit}")
        return self._search_by_vector(query=query, limit=limit)
    ...
    def search_general(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[GENERAL SEARCH FALLBACK FAILED] error={exc}")
            return []
    ...
    def _ensure_payload_indexes_once(self) -> None:
        if BookQdrantSearcher._payload_indexes_ensured:
            return

        BookQdrantSearcher._payload_indexes_ensured = True
        try:
            if not self.client.collection_exists(self.collection_name):
                print(f"[QDRANT PAYLOAD INDEX SKIPPED] collection not found: {self.collection_name}")
                return
        except Exception as exc:
            print(f"[QDRANT PAYLOAD INDEX CHECK SKIPPED] error={exc}")
            return
        ...
        for field_name, schema in self._payload_indexes:
            try:
                ...
                print(f"[QDRANT PAYLOAD INDEX ENSURED] field={field_name}, schema={schema}")
            except Exception as exc:
                # 이미 존재하는 index, Qdrant 버전 차이, 권한 문제는 검색 자체를 막지 않습니다.
                print(f"[QDRANT PAYLOAD INDEX SKIPPED] field={field_name}, reason={exc}")
    ...
    def _search_by_vector(
        ...
        *,
        ...
        query = str(query or "").strip()
        if not query:
            print("[QDRANT VECTOR SEARCH SKIPPED] reason=empty_query")
            return []
        if self.hybrid_enabled:
            # 수정 포인트: BM25 ON 요청에서만 hybrid collection의 dense/sparse 결과를 내부 RRF로 결합합니다.
        ...
        if bool(getattr(settings, "QDRANT_CACHE_ENABLED", True)):
            ...
            if cached is not None:
                ...
                print(
                    f"[QDRANT SEARCH TIMING] collection={self.collection_name} cache_hit=true "
        ...
        if query_vector is None:
            print(f"[QDRANT SEARCH FALLBACK] embedding failed → return empty list embedding_ms={embedding_ms}")
            return []
        ...
        try:
            ...
            print(f"[QDRANT VECTOR SEARCH ERROR] embedding_ms={embedding_ms} qdrant_search_ms={qdrant_ms} error={e}")
            return []

        qdrant_ms = int((perf_counter() - qdrant_started_at) * 1000)
        formatted = self._format_points(response.points, match_type=match_type)
        self._set_retrieval_metadata(
            requested_strategy="dense_lookup",
            used_strategy="dense_lookup",
            fallback=False,
        ...
        print(
            f"[QDRANT SEARCH TIMING] collection={self.collection_name} cache_hit=false "
        ...
        *,
        ...
        if bool(getattr(settings, "QDRANT_CACHE_ENABLED", True)):
            ...
            if cached is not None:
                ...
                print(
                    f"[QDRANT HYBRID SEARCH TIMING] collection={self.collection_name} cache_hit=true "
        ...
        if not sparse_vector.get("indices"):
            print(
                f"[QDRANT HYBRID SPARSE SKIPPED] collection={self.collection_name} "
        ...
        dense_candidates = self._format_points(dense_response, match_type="dense")
        sparse_candidates = self._format_points(sparse_response, match_type="bm25_text")
        fused = rrf_fuse(
        ...
        for item in fused:
            item["match_type"] = "dense_bm25_rrf" if match_type == "vector" else f"{match_type}_dense_bm25_rrf"

        try:
            hydrate_started_at = perf_counter()
            fused = self._hydrate_fused_candidates(fused, match_type="dense_bm25_rrf")
        ...
        print(
            f"[QDRANT HYBRID SEARCH TIMING] collection={self.collection_name} cache_hit=false "
    ...
    def _run_hybrid_search_requests(
        ...
        *,
        ...
        if bool(getattr(settings, "QDRANT_HYBRID_PARALLEL_SEARCH_ENABLED", True)):
            with ThreadPoolExecutor(max_workers=2, thread_name_prefix="qdrant-hybrid") as executor:
    ...
    def _search_named_dense_vector(
        ...
        *,
        ...
        print(
            f"[QDRANT HYBRID DENSE SEARCH TIMING] collection={self.collection_name} "
    ...
    def _fallback_dense_lookup(
        ...
        print(
            f"[QDRANT HYBRID FALLBACK] requested_strategy=dense_bm25_rrf used_strategy=dense_lookup "
        ...
        results = self._search_dense_vector_on_collection(
            ...
            query_filter=query_filter,
            collection_name=self.fallback_collection_name,
            match_type="vector_fallback_dense_lookup",
            cache_namespace="hybrid_fallback_dense",
        )
        self._set_retrieval_metadata(
            requested_strategy="dense_bm25_rrf",
            used_strategy="dense_lookup",
    ...
    def _search_by_keyword(
        ...
        """명확한 payload 검색만 처리합니다.

        기존에는 일반 질의도 client-side scroll로 전체 payload를 순회할 수 있었습니다.
        이 메서드는 author/title/isbn 모드에서만 호출되도록 제한합니다.
        """
            ...
            for field_name in field_names:
                ...
                for query_filter in query_filters:
                    try:
                        ...
                        print(
                            f"[QDRANT PAYLOAD FILTER SEARCH FAILED] "
                            f"mode={search_mode}, field={field_name}, keyword={raw_keyword!r}, error={e}"
                        )
                        continue
                    ...
                    for point in points:
                        ...
                        if not self._payload_matches_keywords(payload, normalized_keywords, search_mode):
                            continue
                        ...
                        if len(matched_points) >= limit:
                            return self._format_points(
                                matched_points,
                                match_type=f"payload_{search_mode}",
                            )

        return self._format_points(matched_points, match_type=f"payload_{search_mode}")
    ...
    def _normalize_keywords(keywords: List[str]) -> List[str]:
        ...
        for keyword in keywords:
            value = BookQdrantSearcher._normalize_text(keyword)
    ...
    def _is_strict_keyword_query(self, query: str) -> bool:
        search_mode = self._detect_search_mode(query)
        return search_mode in {"author", "title", "isbn"}
