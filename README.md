...
3. Pull Request 대상은 기본적으로 dev로 한다.
4. API Key, DB Password, 원본 데이터셋은 절대 Git에 커밋하지 않는다.
5. 환경변수는 .env.example에 변수명만 공유한다.
6. DB 변경사항은 database/ddl 또는 database/migrations에 SQL로 남긴다.
7. 회의/결정사항은 docs/meeting-notes에 기록한다.
...
- **DB**: book_curation
- **Schema**: book
- **DBMS**: PostgreSQL + PostGIS 사용
- **위치 계산**: GEOGRAPHY(POINT, 4326) 사용
